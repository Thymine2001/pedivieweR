...
# Try to load Rcpp QC function (handled by package loading mechanism)
# If use_rcpp is already set in the environment, use it; otherwise try to load
if (!exists("use_rcpp") || !use_rcpp) {
  use_rcpp <- FALSE
  if (requireNamespace("Rcpp", quietly = TRUE)) {
    tryCatch({
      library(Rcpp)
      # Try to load from package src directory
      cpp_file <- system.file("src", "pedigree_qc.cpp", package = "pedivieweR")
      if (file.exists(cpp_file)) {
        if (exists("sourceCpp", envir = parent.env(environment()))) {
          sourceCpp(cpp_file)
        } else {
      ...
      } else if (file.exists("pedigree_qc.cpp")) {
        # Fallback for development
        if (exists("sourceCpp", envir = parent.env(environment()))) {
          sourceCpp("pedigree_qc.cpp")
        } else {
          Rcpp::sourceCpp("pedigree_qc.cpp")
        }
        use_rcpp <- TRUE
        cat("[OK] Rcpp QC functions loaded successfully\n")
      }
...
ui <- page_fillable(
  ...
  div(class = "title-bar",
      h1("PedivieweR 2.0"),
      p("Pedigree Quality Control, Inbreeding Analysis & Interactive Visualization")
  ),
  ...
  div(class = "three-panel-container",
      ...
      div(id = "leftPanel", class = "left-panel",
          ...
          div(class = "panel-section",
              ...
              fileInput("file", "Choose Pedigree File", 
                        accept = c(".csv", ".txt", ".ped")),
              div(
              ...
              hr(),
              checkboxInput("auto_process", "🚀 Auto-detect, calculate F & visualize", value = TRUE),
              helpText("Automatically detect columns, calculate inbreeding, and generate visualization after upload")
          ),
          
          div(class = "panel-section",
              h4("Column Mapping", class = "section-title"),
              uiOutput("col_mapping_ui"),
              uiOutput("data_validation_status_ui"),
              conditionalPanel(
                condition = "!input.auto_process",
                actionButton("process", "Process Data", class = "btn-primary w-100 mt-2")
              ),
              conditionalPanel(
                condition = "input.auto_process",
          ...
          div(class = "panel-section",
              h4("Quick Stats", class = "section-title"),
            ...
            nav_panel(
              ...
              div(style = "margin-top: 10px;",
                  div(style = "margin-bottom: 15px;",
                      textInput("individual_search", "🔍 Search Individual ID:", 
                                placeholder = "Enter individual ID to visualize pedigree",
                                width = "100%"),
                      div(style = "margin-top: 10px;",
                          sliderInput("search_depth", "Search Depth (generations):", 
                                      min = 1, max = 10, value = 5, step = 1,
                                      width = "100%"),
                          actionButton("search_individual", "Visualize", class = "btn-primary"),
                          actionButton("show_highest_f", "Show Highest Inbreeding", class = "btn-secondary"),
                          actionButton("refresh_viz", "🔄 Refresh", class = "btn-secondary")
                      )
                  ),
                  uiOutput("viz_info"),
                  ...
                  div(style = "margin-bottom: 15px; padding: 10px; background: #f8f8f8; border-radius: 5px; border: 1px solid #ddd;",
                      ...
                      div(style = "display: flex; flex-wrap: wrap; gap: 15px;",
                          ...
                          div(style = "display: flex; align-items: center; gap: 5px;",
                              ...
                              span("Target Individual", style = "font-size: 14px;")
                          )
                      ),
                      div(style = "margin-top: 10px; font-size: 14px; color: #666;",
                          "💡 Node size represents inbreeding coefficient (F): Larger nodes = Higher inbreeding")
                      ),
      ...
      div(id = "rightPanel", class = "right-panel",
          ...
          div(class = "panel-section",
              h4("Inbreeding Analysis", class = "section-title"),
              conditionalPanel(
                condition = "!input.auto_process",
              ...
              hr(),
              h5("Top 10 Most Inbred:", style = "font-size: 0.95rem; font-weight: 600;"),
              DTOutput("f_table_top"),
              downloadButton("download_f", "Download All F Values", class = "btn-sm btn-secondary mt-2 w-100")
          ),
          
          div(class = "panel-section",
              h4("Selected Animal Export", class = "section-title"),
              uiOutput("selected_node_info"),
              downloadButton("download_relatives", "📥 Download Selected Range", 
                             class = "btn-primary w-100 mt-2"),
              helpText("Exports: the selected individual and all relatives within the current 'Search Depth (generations)'.")
          ),
          
          div(class = "panel-section",
              h4("Smart Visualization", class = "section-title"),
...
server <- function(input, output, session) {
  ...
  validate_data_format <- function(df, id_col, sire_col, dam_col) {
    ...
    if (length(errors) == 0) {
      ...
      if (anyDuplicated(df[[id_col]])) {
        errors <- c(errors, "ID column contains duplicate values")
      }
      
      # Check if ID column has missing values
      ...
      selectInput("sex_col", "Sex (optional):", 
        ...
        if (is.null(auto_cols$sex) || is.na(auto_cols$sex)) {
          ...
          div(
            ...
            tags$small(
              paste0("[OK] Sex column '", auto_cols$sex, "' detected automatically."),
  ...
  output$download_fixed_pedigree <- downloadHandler(
    filename = function() {
      paste0("fixed_pedigree_", Sys.Date(), ".csv")
    },
    content = function(file) {
      ped <- ped_data()
      if (is.null(ped) || nrow(ped) == 0) {
        write.csv(data.frame(Message = "No processed pedigree available"), file, row.names = FALSE)
        return()
      }
  ...
  # QC Report - highly optimized for large datasets with Rcpp support
  output$qc_report <- renderPrint({
    ...
    if (exists("use_rcpp") && use_rcpp && exists("fast_pedigree_qc")) {
      tryCatch({
        ...
        cat("Total individuals:", format(qc_result$total, big.mark = ","), "\n")
        cat("Founders:", format(qc_result$founders, big.mark = ","), "\n")
        cat("With both parents:", format(qc_result$with_both_parents, big.mark = ","), "\n\n")
        
        if (length(qc_result$duplicate_ids) > 0) {
          cat("⚠️  WARNING: Duplicate IDs:", length(qc_result$duplicate_ids), "\n")
          cat("   Examples:", paste(head(qc_result$duplicate_ids, 5), collapse = ", "), "\n\n")
        } else {
          cat("✔ No duplicate IDs\n\n")
        }
        
        if (qc_result$self_parent_count > 0) {
          cat("⚠️  WARNING: Self-parenting:", qc_result$self_parent_count, "cases\n\n")
        } else {
          cat("✔ No self-parenting issues\n\n")
        }
        
        if (length(qc_result$missing_sires) > 0) {
          cat("⚠️  Missing sires:", length(qc_result$missing_sires), "\n")
          if (length(qc_result$missing_sires) <= 10) {
            cat("   IDs:", paste(qc_result$missing_sires, collapse = ", "), "\n")
          }
        } else {
          cat("✔ All sires found\n")
        }
        
        if (length(qc_result$missing_dams) > 0) {
          cat("⚠️  Missing dams:", length(qc_result$missing_dams), "\n")
          if (length(qc_result$missing_dams) <= 10) {
            cat("   IDs:", paste(qc_result$missing_dams, collapse = ", "), "\n")
          }
        } else {
          cat("✔ All dams found\n")
        }
        ...
        cat("Note: Rcpp QC failed, using R version\n")
        cat("Error:", e$message, "\n\n")
      })
    }
    ...
    # Optimized founder count using vectorized operations
    founders <- sum(is.na(ped$Sire) & is.na(ped$Dam))
    cat("Founders:", format(founders, big.mark = ","), "\n")
    
    # Optimized both parents count
    both_parents <- sum(!is.na(ped$Sire) & !is.na(ped$Dam))
    cat("With both parents:", format(both_parents, big.mark = ","), "\n\n")
    
    # Fast duplicate check using anyDuplicated
    if (anyDuplicated(ped$ID)) {
      dups <- ped$ID[duplicated(ped$ID)]
      cat("⚠️  WARNING: Duplicate IDs:", length(dups), "\n")
      cat("   Examples:", paste(head(dups, 5), collapse = ", "), "\n\n")
    } else {
      cat("✔ No duplicate IDs\n\n")
    }

    # Show last fix summary if available
    fix_info <- last_fix_summary()
    if (!is.null(fix_info)) {
      cat("🛠  Fix Summary (most recent):\n")
      cat("   - Invalid Sire references removed:", fix_info$fixed_sire, "\n")
      cat("   - Invalid Dam references removed:", fix_info$fixed_dam, "\n")
      cat("   - Total parent cells set to NA:", fix_info$total_fixed_cells, "\n")
      cat("   - Time:", as.character(fix_info$when), "\n\n")
    }
    
    # Fast self-parenting check using vectorized comparison
    self_parent_sire <- sum(ped$ID == ped$Sire, na.rm = TRUE)
    self_parent_dam <- sum(ped$ID == ped$Dam, na.rm = TRUE)
    self_parent_total <- self_parent_sire + self_parent_dam
    
    if (self_parent_total > 0) {
      cat("⚠️  WARNING: Self-parenting:", self_parent_total, "cases\n\n")
    } else {
      cat("✔ No self-parenting issues\n\n")
    }
    
    # For large datasets, use sampling for missing parent check
    if (n > 10000) {
      cat("ℹ️  Dataset has", format(n, big.mark = ","), "individuals.\n")
      cat("   Basic QC completed. Detailed missing parent analysis skipped for performance.\n")
      cat("   Tip: Filter to smaller subset if detailed QC needed.\n")
    } else {
      # Fast missing parent check using %in% operator
      unique_ids <- unique(ped$ID)
      
      # Remove NA and empty values efficiently
      valid_sires <- ped$Sire[!is.na(ped$Sire) & ped$Sire != "" & ped$Sire != "0"]
      valid_dams <- ped$Dam[!is.na(ped$Dam) & ped$Dam != "" & ped$Dam != "0"]
      
      if (length(valid_sires) > 0) {
        missing_sires <- setdiff(unique(valid_sires), unique_ids)
        if (length(missing_sires) > 0) {
          cat("⚠️  Missing sires:", length(missing_sires), "\n")
          if (length(missing_sires) <= 10) {
            cat("   IDs:", paste(missing_sires, collapse = ", "), "\n")
          }
        } else {
          cat("✔ All sires found\n")
        }
      ...
      if (length(valid_dams) > 0) {
        missing_dams <- setdiff(unique(valid_dams), unique_ids)
        if (length(missing_dams) > 0) {
          cat("⚠️  Missing dams:", length(missing_dams), "\n")
          if (length(missing_dams) <= 10) {
            cat("   IDs:", paste(missing_dams, collapse = ", "), "\n")
          }
        } else {
          cat("✔ All dams found\n")
        }
...
# Note: shinyApp(ui, server) is called by pedivieweR::run_pedivieweR()
# or pedivieweR::pedivieweR_app() instead of directly executing here
